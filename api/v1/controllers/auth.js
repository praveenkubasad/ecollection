...
module.exports.loginWithEmail = async (req, res) => {
  ...
  if (authUser.role == AccountConstants.accRoles.admin) {
    ...
    if (authUser.status == AccountConstants.accountStatus.normalUser) {
      return loginUser(authUser, Headers.EMAIL_KEY, res, false);
    } else {
      // account status is different than approval
      return res.status(401).json({
...
module.exports.verifyAccByToken = async (req, res) => {
  ...
  const verified =  await JWTHandler.verifyAccessToken(token)  // await Token Controllers.verifyUser(token);
  ...
  const authUser = await Auth.findOne({ _id: verified.data?.authId });
...
async function loginUser(authUser, provider, res, isSignup) {
  ...
  const accessToken = await JWTHandler.genAccessToken(authUser._id,authUser.role);
...
async function _generateNewTokensAndSendBackToClient(authUser, res) {
  const newAccessToken = await JWTHandler.genAccessToken(authUser.email,authUser.role);
...
async function _createNewRefreshTokenIfAboutToExpire(authUser) {
  ...
  authUser.refreshToken = await JWTHandler.genRefreshToken(authUser._id,authUser.role);
  return authUser;
}
