...
module.exports.validateAccountFields = (req,res,next)=>{
  const { error } = Validators.registerAccountsValidation(req.body);
  if (error) return sendError(error.details[0].message, error, res);
   else
    next();
}
...
module.exports.validateDailyCollectionFields = (req,res,next)=>{
  const { error } = Validators.dailycollectionValidation(req.body);
  if (error) return sendError(error.details[0].message, error, res);
   else
    next();
}
